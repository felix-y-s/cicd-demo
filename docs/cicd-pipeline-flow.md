...
위 네 가지 외에, 실제 운영에서는 필수급이지만 이 파이프라인에는
언급되지 않은 항목들이다. 우선순위가 높은 순서대로 정리한다.

| 안전장치 | 지금 상태 | 실무에서 흔한 이유 |
|---|---|---|
| DB 마이그레이션과 배포의 정합성 | 없음 — `prisma migrate deploy`는 `test` job에서만 실행되고, `deploy` job에는 마이그레이션 단계가 없음 | 스키마와 앱 코드가 어긋나면 무중단 배포를 아무리 잘해도 데이터 불일치나 런타임 에러 발생 |
| 동시 배포 방지(concurrency control) | 없음 — 워크플로에 `concurrency` 키 없음 | 연속 push 시 배포가 겹쳐 실행되며 컨테이너 이름 충돌, 레이스 컨디션 발생 가능 |
| 다운스트림 의존성까지 확인하는 헬스체크 | 없음 — `curl http://localhost:3000/`는 앱 프로세스 응답만 확인, DB/Redis/RabbitMQ 연결 상태는 미확인 | 앱은 떠 있지만 인프라 연결이 끊긴 "거짓 성공" 배포를 방지 |
| 시크릿 관리 | 없음 — `/home/deployer/app.env`에 `DATABASE_URL`, `JWT_SECRET` 등이 평문 파일로 저장 | 서버 침해 시 평문 시크릿 유출 방지, 접근 감사 필요 |
| 배포 이력/감사 로그 | 없음 — 어떤 커밋(sha)이 언제 프로덕션에 배포됐는지 조회할 별도 기록 없음 | 장애 발생 시 "언제부터 문제였는지" 역추적에 필수 |
| 배포 알림(Slack 등) | 없음 — 성공/실패가 GitHub Actions 로그 안에만 남음 | 배포가 조용히 실패해도 아무도 모른 채 방치되는 것을 방지 |
| 컨테이너 리소스 제한 | 없음 — `docker run`에 `--memory`, `--cpus` 제한 없음 | 메모리 누수나 트래픽 폭증 시 같은 호스트의 DB 등 다른 서비스까지 영향받는 것을 방지 |

> 위 항목들을 실제로 어떻게 구현하는지(Expand-Contract 마이그레이션
> 패턴, Blue-Green/Canary 무중단 배포 전략 등)는
> [deployment-safeguards-guide.md](./deployment-safeguards-guide.md)에
> 별도로 정리했다. 이 문서는 "지금 파이프라인이 실제로 무엇을
> 하는가"를 다루므로, 아직 구현되지 않은 전략의 상세 가이드는 그
> 문서를 참고.
